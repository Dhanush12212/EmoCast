...
function SubscribedVideos() {
  ...
  const fetchVideos = async (page = '') => {
    ...
    try {
      const response = await axios.get(`${API_URL}/subscribe/fetchVideos`, {
      ...
      // Append new videos to existing state to prevent scroll reset
  ...
  return (
    <>
      ...
      {loading && videos.length > 0 && (
        <p className="text-center text-gray-400 mt-4">Loading more videos...</p>
      )}
    </>
  );
}
